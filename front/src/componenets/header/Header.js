...
class Header extends Component {
  ...
  logout = async () => {
    try {
      const response = await axios.post('http://localhost:8080/emp/logout');
      if (response.status === 200) {
        // 로그아웃 요청이 성공적으로 처리됐다면
        // 리다이렉트 또는 다른 적절한 동작을 수행
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div class="bar">
          <MenuIcon sx={{color:'white', fontSize:45, background:'#0f8bff',borderRadius:1}}></MenuIcon>
          &nbsp;&nbsp;&nbsp;
